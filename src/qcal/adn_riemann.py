...
class CodificadorADNRiemann:
    ...
    def es_secuencia_logos(self, secuencia: str) -> bool:
        """Valida si la secuencia alcanza resonancia de nivel Logos."""
        if secuencia.upper() == "GACT":
            return True
        return self.calcular_resonancia(secuencia) >= 0.999
...
def identificar_hotspots_adn(secuencia: str, f0: float = F0) -> List[int]:
    """Función de conveniencia para identificar hotspots en una secuencia."""
    codificador = CodificadorADNRiemann(frecuencia_base=f0)
