...
# 
# Prepare adelic-bsd repository for SageMath integration
# This script creates all necessary files and structure

set -e  # Exit on error

echo "======================================================================="
echo "  BSD SPECTRAL FRAMEWORK - SAGEMATH INTEGRATION PREPARATION"
echo "======================================================================="
echo ""
...
if [ -d "$INTEGRATION_DIR/sage/schemes/elliptic_curves/bsd_spectral" ]; then
    echo -e "${GREEN}✓ Module structure verified${NC}"
else
    echo -e "${YELLOW}! Module structure not found${NC}"
    exit 1
fi

if [ -d "$INTEGRATION_DIR/doc/en/reference/bsd_spectral" ]; then
    echo -e "${GREEN}✓ Documentation structure verified${NC}"
else
    echo -e "${YELLOW}! Documentation structure not found${NC}"
    exit 1
fi
echo ""

# Step 2: Check module files
echo "Step 2: Checking module files..."
MODULE_FILES=(
    "sage/schemes/elliptic_curves/bsd_spectral/__init__.py"
    "sage/schemes/elliptic_curves/bsd_spectral/spectral_finiteness.py"
    "sage/schemes/elliptic_curves/bsd_spectral/dR_compatibility.py"
    "sage/schemes/elliptic_curves/bsd_spectral/PT_compatibility.py"
    "sage/schemes/elliptic_curves/bsd_spectral/all.py"
)
...
echo "Step 3: Checking documentation files..."
DOC_FILES=(
    "doc/en/reference/bsd_spectral/index.rst"
    "doc/en/reference/bsd_spectral/spectral_finiteness.rst"
    "doc/en/reference/bsd_spectral/dR_compatibility.rst"
    "doc/en/reference/bsd_spectral/PT_compatibility.rst"
)
...
echo "Step 4: Verifying PR template..."
if [ -f "$INTEGRATION_DIR/PULL_REQUEST.md" ]; then
    echo -e "${GREEN}✓ PULL_REQUEST.md exists${NC}"
else
    echo -e "${YELLOW}✗ PULL_REQUEST.md (missing)${NC}"
    exit 1
fi
echo ""

# Step 5: Count doctests
echo "Step 5: Analyzing doctest coverage..."
TOTAL_EXAMPLES=0
TOTAL_TESTS=0

for pyfile in sage/schemes/elliptic_curves/bsd_spectral/*.py; do
    if [ -f "$INTEGRATION_DIR/$pyfile" ]; then
        EXAMPLES=$(grep -c "EXAMPLES::" "$INTEGRATION_DIR/$pyfile" || true)
        TESTS=$(grep -c "TESTS::" "$INTEGRATION_DIR/$pyfile" || true)
        TOTAL_EXAMPLES=$((TOTAL_EXAMPLES + EXAMPLES))
        TOTAL_TESTS=$((TOTAL_TESTS + TESTS))
        
        if [ $EXAMPLES -gt 0 ] || [ $TESTS -gt 0 ]; then
            echo -e "${GREEN}✓ $(basename $pyfile): ${EXAMPLES} EXAMPLES, ${TESTS} TESTS${NC}"
        fi
    fi
done

echo ""
echo -e "${BLUE}Total doctests found:${NC}"
echo -e "  EXAMPLES sections: ${TOTAL_EXAMPLES}"
echo -e "  TESTS sections: ${TOTAL_TESTS}"
echo ""
...
echo -e "${GREEN}✓ PR template ready${NC}"
echo -e "${GREEN}✓ Doctest coverage: ${TOTAL_EXAMPLES} EXAMPLES + ${TOTAL_TESTS} TESTS${NC}"
echo ""
...
echo -e "${GREEN}✓ Integration instructions created${NC}"
echo ""
...
echo "   - Module files in: sage/schemes/elliptic_curves/bsd_spectral/"
echo "   - Documentation in: doc/en/reference/bsd_spectral/"
echo "   - PR template: PULL_REQUEST.md"
echo ""
echo "2. Read integration instructions:"
echo "   - INTEGRATION_INSTRUCTIONS.md"
echo ""
echo "3. Follow the step-by-step guide to submit PR to SageMath"
echo ""
echo "4. For questions, contact: institutoconsciencia@proton.me"
echo ""
echo "======================================================================="
